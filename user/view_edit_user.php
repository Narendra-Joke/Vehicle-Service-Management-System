...
<div class="container-fluid my-3 bg-white rounded">
    ...
    if (isset($_POST['user_update'])) {
        ...
        $update_query = "update `users` set fullname='$fullname',email='$email',contact='$contact',address='$address',image='$image',date_updated=NOW() where username='$_SESSION[username]'";
        $sql_execute = mysqli_query($con, $update_query);
        if ($sql_execute) {
            echo "<script>alert('Profile updated successfully');</script>";
            echo "<script>window.open('./profile.php?view_edit','_self');</script>";
        } else {
